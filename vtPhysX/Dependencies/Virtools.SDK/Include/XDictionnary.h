...
public:
    ...
    protected:
        Node *FillFirstSequence(Node *iNode)
        {
            ...
            if (!(n->m_Flag & Node::TERMINATION))
            { // if the node is not a termination, we add its suffix
...
private:
    ...
    Node *GetNextChild(Node *iNode, int iSymbol)
    {
        ...
        else
        {
            ...
            { // need to child for having a brother
    ...
    {
        if (iNode->m_Flag & Node::SUFFIX)
        { // we transform a final node with suffix in an internal node with children
    ...
    bool _InsertSequence(Node *iNode, const T *iSequence, const int iCount, void *iUserData, bool iReplace)
    {
        ...
        for (int i = 0; i < iCount; ++i)
        {
            ...
            Node *&child = n->Children.data[symbol - m_SymbolOffset];
    ...
    int _RemoveSequence(Node *iNode, const T *iSequence, const int iCount)
    {
        ...
        if (iCount && iNode->HasChildren())
        {
            ...
            if (_RemoveSequence(child, iSequence + 1, iCount - 1))
            { // my child is no good anymore
                DestroyNode(child);
                child = NULL;
        ...
        { // Suffix
            ...
            else
            {
                ...
                if (iNode->m_Flag & Node::TERMINATION)
                { // the node was also a termination
