...
private:
    ...
    {
        if (!iRoot)
        { // we insert at the root of the tree
            m_Roots.PushBack(iNode);
        }
        else
        { // we insert under an existing node
    ...
    void _DeleteNode(Node *iRoot)
    {
        ...
        if (iRoot->type == Node::NODE)
        { // If we are an intenral Node
            INode *inode = (INode *)iRoot;
