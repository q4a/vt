   ...
   ************************************************/
    ...
    ************************************************/
    ...
    Iterator Erase(Iterator iT)
    {
        ...
        if (XLeft(y) == m_Nil)
            x = XRight(y);
        else if (XRight(y) == m_Nil)
            x = XLeft(y);
        else
            y = XMin(XRight(y)), x = XRight(y);
...
protected:
    ...
    static Node *XMax(Node *iN)
    {
        while (iN->right != m_Nil)
            iN = iN->right;
        return iN;
    }
...
private:
    ...
    {
        if ((t->m_Left == m_NullNode) && (t->m_Right == m_NullNode))
            return t->m_Data;
        if (t->m_Left != m_NullNode)
            return XGetSmallest(t->m_Left);
        if (t->m_Right != m_NullNode)
            return XGetSmallest(t->m_Right);
        return t->m_Data;
    }
